...
(define mult-and-neg-deffuns
  (list
   ...
   '(deffun (neg? x) (split-neg? x x))
   '(deffun (split-neg? p n) (if0 (or (zero? p)(zero? n)) 1
      (if0 (+ n 1) 0 (if0 (zero? (- p 1)) 1 (split-neg? (- p 1) (+ n 1))))))
...
; create a value with the same name as a function
(test
  (fnwae-interp-plus (parse '{+ {f -5} {with {f 100}{f f}}}) 
          (list (parse-defn 
                 '{deffun {f x} {if0 {neg? {- x 20}}
                                     {with {x {+ x 1}} {f x}} 
                                     x}})))
  120)
; same thing but with a no arg function
(test
  (fnwae-interp-plus (parse '{with {f 100}{f}}) 
                     (list (parse-defn '{deffun {f} 5})))
  5)

; same thing but dont call the function
(test
  (fnwae-interp-plus (parse '{with {f 100} f}) 
                     (list (parse-defn '{deffun {f} 5})))
  100)
...
;; bad function call from a function
(test/exn
  (fnwae-interp-plus (parse '{f}) 
                     (list (parse-defn '{deffun {f} {z}})))
  "undefined function: z")

;; bad function call from a function, but with a z defined in a with
(test/exn
  (fnwae-interp-plus (parse '{with {z 10} {f}}) 
                     (list (parse-defn '{deffun {f} {z}})))
  "undefined function: z")
